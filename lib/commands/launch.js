...
module.exports = async function (cb, {nomin = false}) {
	...
	const apexProxyRequest = function (request, response) {
		...
		request.setHeader('Origin', request.agent.protocol + '//' + (request.connection._host || request._headers.host) + ':' + (config.launch.port || 4000) );
	...
	const launch = () => {
		const browsersyncOptions = {
			...
			proxy: {
				...
				middleware: [compress()] //Added compression to fix issue with Cypress running on top of browsersync proxy that emits chunked encoding and content-length causing Cypress to fail
			},
			serveStatic: [path.resolve(config.distFolder || config.srcFolder)],
			ui: false,
			logLeveL: config.launch.logLevel || "info",
